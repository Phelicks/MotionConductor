...
//  Tracing & Debugging
// ------------------------------------------------------------------------------------
#define PREFIX	TEXT("WiiYourself! : ")
...
# define TRACE(fmt, ...) _TRACE(PREFIX          fmt          TEXT("\n"), __VA_ARGS__)
# define WARN(fmt, ...)  _TRACE(PREFIX TEXT("* ") fmt TEXT(" *") TEXT("\n"), __VA_ARGS__)
#elif defined(__MINGW32__)
# define TRACE(fmt, ...) _TRACE(PREFIX          fmt          TEXT("\n") , ##__VA_ARGS__)
# define WARN(fmt, ...)  _TRACE(PREFIX TEXT("* ") fmt TEXT(" *") TEXT("\n") , ##__VA_ARGS__)
#endif
// uncomment any of these for deeper debugging:
//#define DEEP_TRACE(fmt, ...) _TRACE(PREFIX TEXT("|") fmt TEXT("\n"), __VA_ARGS__)    // VC 2005+
//#define DEEP_TRACE(fmt, ...) _TRACE(PREFIX TEXT("|") fmt TEXT("\n") , ##__VA_ARGS__) // mingw
...
const TCHAR*   wiimote::ButtonNameFromBit		 [TOTAL_BUTTON_BITS] =
                                { TEXT("Left") , TEXT("Right"), TEXT("Down"), TEXT("Up"),
                                  TEXT("Plus") , TEXT("??")   , TEXT("??")  , TEXT("??") ,
                                  TEXT("Two")  , TEXT("One")  , TEXT("B")   , TEXT("A") ,
                                  TEXT("Minus"), TEXT("??")   , TEXT("??")  , TEXT("Home") };

const TCHAR*   wiimote::ClassicButtonNameFromBit [TOTAL_BUTTON_BITS] =
                                { TEXT("??")   , TEXT("TrigR")  , TEXT("Plus") , TEXT("Home"),
                                  TEXT("Minus"), TEXT("TrigL") , TEXT("Down") , TEXT("Right") ,
                                  TEXT("Up")   , TEXT("Left")   , TEXT("ZR")   , TEXT("X") ,
                                  TEXT("A")    , TEXT("Y")      , TEXT("B")    , TEXT("ZL") };
// ------------------------------------------------------------------------------------
wiimote::wiimote ()
	:
	...
	if(++_TotalCreated == 1)
		{
        HidDLL = LoadLibrary(TEXT("hid.dll"));
		_ASSERT(HidDLL);
		if(!HidDLL)
            WARN(TEXT("Couldn't load hid.dll - shouldn't happen!"));
		else{
			_HidD_SetOutputReport = (hidwrite_ptr)
									GetProcAddress(HidDLL, "HidD_SetOutputReport");
			if(_HidD_SetOutputReport)
                TRACE(TEXT("OS supports HID writes."));
			else
                TRACE(TEXT("OS doesn't support HID writes."));
			}
		}
...
bool wiimote::Connect (unsigned wiimote_index, bool force_hidwrites)
	...
	if(wiimote_index == FIRST_AVAILABLE)
        TRACE(TEXT("Connecting first available Wiimote:"));
	else
        TRACE(TEXT("Connecting Wiimote %u:"), wiimote_index);
	...
	if(!dev_info) {
        WARN(TEXT("couldn't get device info"));
		return false;
		}
	...
	while(SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
		...
		if(HidD_GetAttributes(Handle, &attrib))
			...
			if(wiimote_index == FIRST_AVAILABLE)
                TRACE(TEXT(".. opening Wiimote %u:"), wiimotes_found);
			else
                TRACE(TEXT(".. opening:"));
			...
			if(Handle == INVALID_HANDLE_VALUE) {
                TRACE(TEXT(".... failed with err %x"), GetLastError());
				goto skip;
				}
			...
			if(force_hidwrites && !_HidD_SetOutputReport) {
                TRACE(TEXT(".. can't force HID writes (not supported)"));
				force_hidwrites = false;
				}

			if(force_hidwrites)
                TRACE(TEXT(".. (HID writes forced)"));
			else{
				...
				while(!bStatusReceived && ((timeGetTime()-last_time) < 500))
					Sleep(10);
                TRACE(TEXT(".. WriteFile() %s."), bStatusReceived? TEXT("succeeded") :
                                                                 TEXT("failed"));
				}
			...
			if(!bStatusReceived && _HidD_SetOutputReport)
				...
				while(!bStatusReceived && ((timeGetTime()-last_time) < 500))
					Sleep(10);
				// did we get it?
                TRACE(TEXT(".. HID write %s."), bStatusReceived? TEXT("succeeded") :
                                                               TEXT("failed"));
				}

			// still failed?
			if(!bStatusReceived) {
                WARN(TEXT("output failed - wiimote is not connected (or confused)."));
				Disconnect();
				goto skip;
				}
...
void wiimote::CalibrateAtRest ()
	...
	if(IsBalanceBoard()) {
        TRACE(TEXT(".. removing 'at rest' BBoard offsets."));
...
unsigned __stdcall wiimote::ReadParseThreadfunc (void* param)
	...
	while(1)
		...
		if(remote.Handle == INVALID_HANDLE_VALUE) {
            DEEP_TRACE(TEXT("read thread: wiimote was disconnected"));
			break;
			}
		...
		if(remote.bConnectionLost)
			{
connection_lost:
            TRACE(TEXT("read thread: connection to wiimote was lost"));
			remote.Disconnect();
...
//			   !remote.bConnectInProgress   &&
			   ...
			   (remote.Internal.ExtensionType != MOTION_PLUS2) &&
		...
		if(wait == WAIT_TIMEOUT) {
            DEEP_TRACE(TEXT("read thread: timed out"));
			continue; // wait again
			}
		//  did an error occurr?
		if(wait != WAIT_OBJECT_0) {
            DEEP_TRACE(TEXT("read thread: error waiting!"));
		...
		if(read) {
            DEEP_TRACE(TEXT("read thread: parsing data"));
			remote.OnReadData(read);
			}
		else
            DEEP_TRACE(TEXT("read thread: didn't get any data??"));
		}

    TRACE(TEXT("(ending read thread)"));
#ifdef BEEP_DEBUG_READS
	if(exit_code != 0)
		Beep(200,1000);
#endif
...
bool wiimote::BeginAsyncRead ()
    ...
    DEEP_TRACE(TEXT(".. starting async read"));
#ifdef BEEP_DEBUG_READS
	Beep(1000,1);
#endif
	...
	if (!ReadFile(Handle, ReadBuff, REPORT_LENGTH, &read, &Overlapped)) {
		...
		if(err != ERROR_IO_PENDING) {
            DEEP_TRACE(TEXT(".... ** ReadFile() failed! **"));
			return false;
			}
		}
	...
	if(!ReadParseThread) {
		ReadParseThread = (HANDLE)_beginthreadex(NULL, 0, ReadParseThreadfunc,
												 this, 0, NULL);
        DEEP_TRACE(TEXT(".... creating read thread"));
	...
	if(read) {
        DEEP_TRACE(TEXT(".... got data right away"));
		SetEvent(DataRead);
		}
	return true;
	}
...
void wiimote::SetReportType (input_report type, bool continuous)
...
#ifdef TRACE
    #define TYPE2NAME(_type)	(type==_type)? TEXT(#_type)
	const TCHAR* name = TYPE2NAME(IN_BUTTONS)				:
                        ...
                        TEXT("(unknown??)");
    TRACE(TEXT("ReportType: %s (%s)"), name, (continuous? TEXT("continuous") :
                                                        TEXT("non-continuous")));
#endif
	ReportType = type;
...
void wiimote::RumbleForAsync (unsigned milliseconds)
	...
	if(!AsyncRumbleThread) {
        WARN(TEXT("couldn't create rumble thread!"));
		return;
		}
...
int wiimote::ParseInput (BYTE* buff)
	...
	switch(buff[0])
		{
		case IN_BUTTONS:
            DEEP_TRACE(TEXT(".. parsing buttons."));
			changed |= ParseButtons(buff);
			break;

		case IN_BUTTONS_ACCEL:
            DEEP_TRACE(TEXT(".. parsing buttons/accel."));
			changed |= ParseButtons(buff);
			if(!IsBalanceBoard())
		...
		case IN_BUTTONS_ACCEL_EXT:
            DEEP_TRACE(TEXT(".. parsing extenion/accel."));
			changed |= ParseButtons(buff);
		...
		case IN_BUTTONS_ACCEL_IR:
            DEEP_TRACE(TEXT(".. parsing ir/accel."));
			changed |= ParseButtons(buff);
			if(!IsBalanceBoard()) {
				changed |= ParseAccel(buff);
		...
		case IN_BUTTONS_ACCEL_IR_EXT:
            DEEP_TRACE(TEXT(".. parsing ir/extenion/accel."));
			changed |= ParseButtons(buff);
		...
		case IN_BUTTONS_BALANCE_BOARD:
            DEEP_TRACE(TEXT(".. parsing buttson/balance."));
			changed |= ParseButtons(buff);
			changed |= ParseExtension(buff, 3);
			break;

		case IN_READADDRESS:
            DEEP_TRACE(TEXT(".. parsing read address."));
			changed |= ParseButtons	   (buff);
			changed |= ParseReadAddress(buff);
			break;

		case IN_STATUS:
            DEEP_TRACE(TEXT(".. parsing status."));
		...
		default:
            DEEP_TRACE(TEXT(".. ** unknown input ** (happens)."));
			///_ASSERT(0);
	...
	if(Recording.bEnabled && (changed & Recording.TriggerFlags))
		{
        DEEP_TRACE(TEXT(".. adding state to history"));
		state_event event;
...
void wiimote::DetectMotionPlusExtensionAsync ()
	{
#ifdef _DEBUG
    TRACE(TEXT("(looking for motion plus)"));
#endif
...
bool wiimote::EnableMotionPlus ()
    ...
    TRACE(TEXT("Enabling Motion Plus:"));
...
bool wiimote::DisableMotionPlus ()
    ...
    TRACE(TEXT("Disabling Motion Plus:"));
...
void wiimote::InitializeExtension ()
	{
    TRACE(TEXT("Initialising Extension."));
...
//	TRACE(TEXT("(extension = %s)"), (extension? TEXT("TRUE") : TEXT("false")));

	if(extension != Internal.bExtension)
		{
		if(!Internal.bExtension)
			{
            TRACE(TEXT("Extension connected:"));
			Internal.bExtension = true;
			InitializeExtension();
			}
		else{
            TRACE(TEXT("Extension disconnected."));
...
int wiimote::ParseExtension (BYTE *buff, unsigned offset)
	...
	switch(Internal.ExtensionType)
		...
		case BALANCE_BOARD:
			...
			if(bCalibrateAtRest) {
				bCalibrateAtRest = false;
                TRACE(TEXT(".. Auto-removing 'at rest' BBoard offsets."));
				Internal.BalanceBoard.AtRestKg = Internal.BalanceBoard.Kg;
				}
		...
		case MOTION_PLUS:
			...
			if((yaw != 0x3fff) || (pitch != 0x3fff) || (roll != 0x3fff))
				...
				if(extension != bMotionPlusExtension)
					{
					if(extension) {
                        TRACE(TEXT(".. MotionPlus extension found."));
						changed |= MOTIONPLUS_EXTENSION_CONNECTED;
						}
					else{
                        TRACE(TEXT(".. MotionPlus' extension disconnected."));
...
int wiimote::ParseReadAddress (BYTE* buff)
	...
	if((buff[3] & 0x08) != 0) {
        WARN(TEXT("error: read address not valid."));
		_ASSERT(0);
		return NO_CHANGE;
		}
	// address read failed (write-only)?
		...
		if(MotionPlusDetectCount)
			{
			--MotionPlusDetectCount;
			if (Internal.ExtensionType == MOTION_PLUS || Internal.ExtensionType == MOTION_PLUS2)
			{
				if(bMotionPlusDetected)
                    TRACE(TEXT(".. MotionPlus removed."));
				bMotionPlusDetected  = false;
				bMotionPlusEnabled   = false;
...
//					InitializeExtension();
            ...
            WARN(TEXT("error: attempt to read from write-only register 0x%X."), buff[3]);
	...
	switch(address)
		...
		case (REGISTER_CALIBRATION & 0xffff):
			{
			_ASSERT(size == 6);
            TRACE(TEXT(".. got wiimote calibration."));
...
//			TRACE(TEXT("(found extension 0x%I64x)"), type);
			...
			if ((type == MOTION_PLUS_DETECT) || (type == MOTION_PLUS_DETECT2) || (type == MOTION_PLUS_DETECT3) || (type == MOTION_PLUS_DETECT4))
				{
				if(!bMotionPlusDetected) {
                    TRACE(TEXT("Motion Plus detected!"));
...
#define IF_TYPE(id)	if(type == id) { \
			...
			IF_TYPE(MOTION_PLUS2)
                TRACE(TEXT(".. Motion Plus 2! Switching..."));
				type = MOTION_PLUS; 
				}

			IF_TYPE(MOTION_PLUS)
                TRACE(TEXT(".. Motion Plus!"));
				...
				// sometimes it comes in more than once?
                TRACE(TEXT(".. GH3/GHWT Guitar Controller!"));
				bMotionPlusEnabled = false;
				// and start a query for the calibration data
				ReadAddress(REGISTER_EXTENSION_CALIBRATION, 16);
				}
			else IF_TYPE(GHWT_DRUMS)
                TRACE(TEXT(".. GHWT Drums!"));
				bMotionPlusEnabled = false;
				// and start a query for the calibration data
				ReadAddress(REGISTER_EXTENSION_CALIBRATION, 16);
				}
			else IF_TYPE(BALANCE_BOARD)
                TRACE(TEXT(".. Balance Board!"));
				bMotionPlusEnabled = false;
				// and start a query for the calibration data
                ...
                TRACE(TEXT("unknown extension controller found (0x%I64x)"), type);
				}
			}
			break;
...
void wiimote::ReadCalibration ()
	{
    TRACE(TEXT("Requestion wiimote calibration:"));
...
unsigned __stdcall wiimote::HIDwriteThreadfunc (void* param)
	{
	_ASSERT(param);
    TRACE(TEXT("(starting HID write thread)"));
	wiimote &remote = *(wiimote*)param;
...
remove_entry:
    ...
    TRACE(TEXT("ending HID write thread"));
	return 0;
	}
// ------------------------------------------------------------------------------------
bool wiimote::WriteReport (BYTE *buff)
...
#ifdef _DEBUG
    #define DEEP_TRACE_TYPE(type)	case OUT_##type: DEEP_TRACE(TEXT("WriteReport: ")\
                                                                TEXT(#type)); break
	switch(buff[0])
		{
		...
		DEEP_TRACE_TYPE(IR2);
		default:
            TRACE(TEXT("WriteReport: type [%02x][%02x]"), buff[1], buff[2]);
		}
#endif
	...
	if(bUseHIDwrite)
		...
		if(!HIDwriteThread)
			...
			if(!HIDwriteThread) {
                WARN(TEXT("couldn't create HID write thread!"));
				return false;
				}
	...
	if(!WriteFile(Handle, buff, REPORT_LENGTH, &written, &Overlapped))
		...
		if(error != ERROR_IO_PENDING) {
            TRACE(TEXT("WriteFile failed, err: %u!"), error);
...
bool wiimote::MuteSpeaker (bool on)
    ...
    if(on) TRACE(TEXT("muting speaker."  ));
    else   TRACE(TEXT("unmuting speaker."));
...
bool wiimote::EnableSpeaker (bool on)
    ...
    if(on) TRACE(TEXT("enabling speaker.")); else TRACE(TEXT("disabling speaker."));
...
unsigned __stdcall wiimote::SampleStreamThreadfunc (void* param)
	{
    TRACE(TEXT("(starting sample thread)"));
...
//				}
    ...
    TRACE(TEXT("(ending sample thread)"));
	return 0;
	}
// ------------------------------------------------------------------------------------
bool wiimote::Load16bitMonoSampleWAV (const TCHAR* filepath, wiimote_sample &out)
    ...
    TRACE(TEXT("Loading '%s'"), filepath);

	FILE *file;
#if (_MSC_VER >= 1400) // VC 2005+
    //_tfopen_s(&file, filepath, TEXT("rb"));
#else
    //file = _tfopen(filepath, TEXT("rb"));
#endif
	_ASSERT(file);
	if(!file) {
        WARN(TEXT("Couldn't open '%s"), filepath);
		return false;
		}
	...
	while(1)
		...
		if(!strncmp(chunk_header.ckID, "fmt ", 4))
			...
			if(format != WAVE_FORMAT_PCM) {
                TRACE(TEXT(".. not uncompressed PCM"));
				goto unsupported;
				}

			// must be mono, 16bit
			if((wf.x.nChannels != 1) || (wf.x.wBitsPerSample != 16)) {
                TRACE(TEXT(".. %d bit, %d channel%s"), wf.x.wBitsPerSample,
													 wf.x.nChannels,
                                                    (wf.x.nChannels>1? TEXT("s"):TEXT("")));
				goto unsupported;
				}
			...
			for(unsigned index=1; index<ARRAY_ENTRIES(FreqLookup); index++)
				...
				if((sample_freq+epsilon) >= FreqLookup[index] &&
				   (sample_freq-epsilon) <= FreqLookup[index]) {
                    ...
                    TRACE(TEXT(".. using speaker freq %u"), FreqLookup[index]);
					break;
					}
				}
			if(freq == FREQ_NONE) {
                WARN(TEXT("Couldn't (loosely) match .wav samplerate %u Hz to speaker"),
					 sample_freq);
				goto unsupported;
				}
			}
			...
			if(read != total_samples)
                ...
                WARN(TEXT("found %s .wav audio data than expected (%u/%u samples)"),
                    ((read < total_samples)? TEXT("less") : TEXT("more")),
					read, total_samples);
...
corrupt_file:
    WARN(TEXT(".wav file is corrupt"));
	fclose(file);
	return false;

unsupported:
    WARN(TEXT(".wav file format not supported (must be mono 16bit PCM)"));
	fclose(file);
	return false;
	}
// ------------------------------------------------------------------------------------
bool wiimote::Load16BitMonoSampleRAW (const TCHAR*   filepath,
    ...
    //  WARN(TEXT("couldn't get filesize for '%s'"), filepath);
    //	return false;
    //	}
	...
	_ASSERT(len);
	if(!len) {
        WARN(TEXT("zero-size sample file '%s'"), filepath);
		return false;
		}
	...
	_ASSERT(samples);
	if(!samples) {
        TRACE(TEXT("Couldn't open '%s"), filepath);
		return false;
		}
	...
	fclose(file);
	if(!res) {
        WARN(TEXT("Couldn't load file '%s'"), filepath);
		goto error;
		}
...
bool wiimote::StartSampleThread ()
	...
	if(!SampleThread) {
        WARN(TEXT("couldn't create sample thread!"));
		MuteSpeaker  (true);
		EnableSpeaker(false);	
		return false;
		}
...
bool wiimote::PlaySquareWave (speaker_freq freq, BYTE volume)
    ...
    TRACE(TEXT("playing square wave."));
